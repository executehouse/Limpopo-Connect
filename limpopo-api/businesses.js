...
// PUT update a business listing (protected)
app.put('/api/businesses/:id', authenticateToken, async (req, res) => {
  ...
  try {
    // First, verify the user owns the business
    const existingBusinessResult = await query('SELECT owner_id FROM listings WHERE id = $1', [id]);
    if (existingBusinessResult.rows.length === 0) {
      return res.status(404).json({ error: 'Business not found.' });
    }

    const business = existingBusinessResult.rows[0];
    if (business.owner_id !== ownerId) {
      return res.status(403).json({ error: 'Forbidden: You do not have permission to update this listing.' });
    }

    // Dynamically build the update query based on provided fields
    const fields = { name, category, description, address, phone, email, website };
    const setClauses = [];
    const values = [];
    let paramIndex = 1;
    ...
    values.push(id);
    const updateQuery = `UPDATE listings SET ${setClauses.join(', ')} WHERE id = $${paramIndex} RETURNING *`;
...
// DELETE a business listing (protected)
